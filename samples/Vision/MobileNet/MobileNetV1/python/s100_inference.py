...
def print_topk_predictions(output: np.ndarray,
    ...
    """
    # Reshape output: from (N, 1, 1) to (N,)
    if len(output.shape) > 1:
        output = output.reshape(-1)
    
    # Model output is already probability distribution, use directly
    ...
    for i in range(topk):
        idx = topk_idx[i].item()  # Use .item() to avoid deprecation warning
        prob = topk_prob[i].item()  # Use .item() to avoid deprecation warning
...
def print_top1_prediction(output: np.ndarray, idx2label: dict = None) -> tuple:
    """
    Print top-1 classification prediction and return prediction info.
    """
    # Reshape output: from (N, 1, 1) to (N,)
    if len(output.shape) > 1:
        output = output.reshape(-1)
    ...
    # Top-1 index and probability
    top_idx = np.argmax(probabilities)
    top_prob = probabilities[top_idx].item()
    
    # Get label name
    label = idx2label[int(top_idx)] if idx2label and int(top_idx) in idx2label else f"Class {int(top_idx)}"
    
    print(f"Top-1 Prediction: {label} (Confidence: {top_prob:.4f})")
    
    return int(top_idx), top_prob, label
...
class MobileNetV1:
    """
    @brief Wrapper class for running inference using a MobileNetV1 model through HB_HBMRuntime.
    """

    def __init__(self, opt):
        """
        @brief Initialize the MobileNetV1 model with model path and extract I/O details.
...
def main() -> None:
    """
    @brief Main function to perform image classification using MobileNetV1.
    ...
    parser.add_argument('--model-path', type=str,
                        default='./model/mobilenetv1_224x224_nv12.hbm',
    ...
    # Initialize MobileNetV1 model instance
    mobilenetv1 = MobileNetV1(opt)
