...
python3 eval_yolo26_det.py \
  --model-path ../model/yolo26n_det_640x640_nv12.hbm \
...
python3 eval_yolo26_cls.py \
  --model-path ../model/yolo26n_cls_224x224_nv12.hbm \
...
python3 eval_yolo26_seg.py \
  --model-path ../model/yolo26n_seg_640x640_nv12.hbm \
  --image-dir ../../../../datasets/coco/val2017 \
  --annotation ../../../../datasets/coco/annotations/instances_val2017.json
```
...
python3 eval_yolo26_pose.py \
  --model-path ../model/yolo26n_pose_640x640_nv12.hbm \
  --image-dir ../../../../datasets/coco/val2017 \
...
- **BPU Task Latency / Throughput**:
  ...
  - **Test Command**: Uses the `hrt_model_exec` tool from the development kit: `hrt_model_exec perf --thread_num 2 --model_file <model.bin>`. This tool measures the time from task submission to completion and accounts for cache warmup.
