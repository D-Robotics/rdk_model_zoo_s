...
- [EfficientNet\_lite0](#efficientnet_lite0)
...
|----------------------|--------------------|---------|------------|----------|-----------------|------------------------------------|------------------------------------|-------------|
| EfficientNet_lite2   | 260x260            | 1000    | 6.1        | 77.6     | 77.0               | 0.565 ms                           | 0.935 ms                           | 3123.682 FPS |
...
3. FP/Quantized Top-1: FP Top-1 is the inference accuracy of the ONNX model before quantization, while Quantized Top-1 is the actual inference accuracy after quantization.
...
```shell
wget https://archive.d-robotics.cc/downloads/rdk_model_zoo/rdk_s100/EfficientNet/efficientnet_lite2_260x260_nv12.hbm
```
...
After installing the necessary libraries, you can use the [get_efficientnet_lite2_onnx.py](python/get_efficientnet_lite0_onnx.py) script in the python folder to download the ONNX file.
...
* If you do not want to configure a terminal proxy, you can manually download the model from [timm/tf_efficientnet_lite2.in1k](https://huggingface.co/timm/tf_efficientnet_lite2.in1k) and use the [python/timm2onnx.py](python/timm2onnx_local.py) script to convert to ONNX.
...
Simplified model saved to tf_efficientnet_lite2.onnx
Total number of parameters in the model: 6063026
```
...
- Frame count is: 200
Perf result:
- Frame totally latency is: 113.028 ms
- Average latency is: 0.565 ms
- Frame rate is: 1702.519 FPS
```
thread_num = 3
```
Running condition:
...
- Frame count is: 200
Perf result:
- Frame totally latency is: 187.063 ms
- Average latency is: 0.935 ms
- Frame rate is: 3123.682 FPS
```
...
```shell
hb_compile --model tf_efficientnet_lite2.onnx --march nash-e
```
...
```shell
hb_compile --config yaml/efficientnet_lite2_config.yaml
```

After compilation, you will find the output in the model_output directory. The file needed for deployment is efficientnet_lite2_260x260_nv12.hbm.
...
Summary:
FPS (1 core): 2407.18
latency: 0.42 ms (415.4 us)
BPU conv original OPs per run: 1,719,472,544
```
...
```shell
python3 python/x86_inference.py -m model_output/efficientnet_lite2_260x260_nv12_quantized_model.bc -i data/zebra_cls.jpg
```

To perform accuracy validation with `x86_inference.py`, use the `--validate` flag to enable validation mode. Example:
