...
| -----------  | ------- | ---- | ------ | ----- | ----- | ----------- | ----------- | ------ |
| EfficientNet_lite0   | 224x224 | 1000 | 4.7    | 75.1 | 74.6 | 0.448       | 0.591        | 4827.886 |
...
安装完必要的库后可以使用python文件夹下的 [get_efficientnet_lite0_onnx.py](python/get_efficientnet_lite0_onnx.py) 脚本完成onnx文件的下载
...
* 若不想配置终端代理可选择手动前往 [timm/tf_efficientnet_lite0.in1k](https://huggingface.co/timm/tf_efficientnet_lite0.in1k) 下载模型 并使用 [python/timm2onnx.py](python/timm2onnx_local.py) 脚本完成onnx转换
...
在下载完毕 .hbm 文件后，可以执行 'test_EfficientNet_lite0.ipynb' 或 python文件夹中的 's100_inference.py' ，在板端实际运行体验实际测试效果。
...
完成模型编译后可在 model_output 目录下发现编译产物，需要上板使用的为 efficientnet_lite0_224x224_nv12.hbm 
