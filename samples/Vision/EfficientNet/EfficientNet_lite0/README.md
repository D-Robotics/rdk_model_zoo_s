...
3. FP32/INT8 Top-1: FP32 Top-1 refers to the inference accuracy of the ONNX model before quantization, while INT8 Top-1 refers to the actual inference accuracy after quantization.
...
```shell
wget https://archive.d-robotics.cc/downloads/rdk_model_zoo/rdk_s100/EfficientNet/efficientnet_lite0_224x224_nv12.hbm
```
...
After installing the necessary libraries, you can use the [get_efficientnet_lite0_onnx.py](python/get_efficientnet_lite0_onnx.py) script in the python folder to download the ONNX file.
...
* If you do not want to configure a terminal proxy, you can manually download the model from [timm/tf_efficientnet_lite0.in1k](https://huggingface.co/timm/tf_efficientnet_lite0.in1k) and use the [python/timm2onnx.py](python/timm2onnx_local.py) script to convert to ONNX.
...
Simplified model saved to tf_efficientnet_lite0.onnx
Total number of parameters in the model: 4631002
```
...
- Frame count is: 200
Perf result:
- Frame totally latency is: 89.637 ms
- Average latency is: 0.448 ms
- Frame rate is: 2107.815 FPS
```
thread_num = 3
```
Running condition:
...
- Frame count is: 200
Perf result:
- Frame totally latency is: 118.219 ms
- Average latency is: 0.591 ms
- Frame rate is: 4827.886 FPS
```
...
```shell
hb_compile --model tf_efficientnet_lite0.onnx --march nash-e
```
...
```shell
hb_compile --config yaml/efficientnet_lite0_config.yaml
```

After compilation, you will find the output in the model_output directory. The file needed for deployment is efficientnet_lite0_224x224_nv12.hbm.
...
```shell
python3 python/x86_inference.py -m model_output/efficientnet_lite0_224x224_nv12_quantized_model.bc -i data/zebra_cls.jpg
```

To perform accuracy validation with `x86_inference.py`, use the `--validate` option to enable validation mode. Example:

```shell
python3 python/x86_inference.py -m model_output/efficientnet_lite0_224x224_nv12_quantized_model.bc --validate -d ../../../imagenet/val -l ../../../imagenet/val.txt
```
