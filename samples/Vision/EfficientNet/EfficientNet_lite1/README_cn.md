...
| -----------  | ------- | ---- | ------ | ----- | ----- | ----------- | ----------- | ------ |
| EfficientNet_lite1   | 240x240 | 1000 | 5.4M    | 76.7 | 76.2 | 0.489       | 0.708        | 4086.470 |
...
安装完必要的库后可以使用python文件夹下的 [get_efficientnet_lite1_onnx.py](python/get_efficientnet_lite1_onnx.py) 脚本完成onnx文件的下载
...
* 若不想配置终端代理可选择手动前往 [timm/tf_efficientnet_lite1.in1k](https://huggingface.co/timm/tf_efficientnet_lite1.in1k) 下载模型 并使用 [python/timm2onnx.py](python/timm2onnx_local.py) 脚本完成onnx转换
...
在下载完毕 .hbm 文件后，可以执行 'test_EfficientNet_lite0.ipynb' 或 pyhton文件夹中的 's100_inference.py' ，在板端实际运行体验实际测试效果。
...
完成模型编译后可在 model_output 目录下发现编译产物，需要上板使用的为 efficientnet_lite1_240x240_nv12.hbm 
