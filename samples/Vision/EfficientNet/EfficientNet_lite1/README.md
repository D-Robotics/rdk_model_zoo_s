...
- [EfficientNet\_lite1](#efficientnet_lite1)
...
| Model                | Input Size (pixels) | Classes | Parameters (M) | FP32 Top-1 | INT8 Top-1 | Latency/Throughput (Single Thread) | Latency/Throughput (Multi Thread) | FPS      |
|----------------------|--------------------|---------|---------------|------------|------------|-------------------------------------|------------------------------------|----------|
| EfficientNet_lite1   | 240x240            | 1000    | 5.4M          | 76.7       | 76.2          | 0.489 ms                            | 0.708 ms                           | 4086.470 |
...
After installing the necessary libraries, you can use the [get_efficientnet_lite1_onnx.py](python/get_efficientnet_lite1_onnx.py) script in the python folder to download the ONNX file.
...
* If you do not want to configure a terminal proxy, you can manually download the model from [timm/tf_efficientnet_lite1.in1k](https://huggingface.co/timm/tf_efficientnet_lite1.in1k) and use the [python/timm2onnx.py](python/timm2onnx_local.py) script to convert to ONNX.
...
Simplified model saved to tf_efficientnet_lite1.onnx
Total number of parameters in the model: 5389850
```

## 4. Deployment Test

After downloading the .hbm file, you can run 'test_EfficientNet_lite1.ipynb' or 's100_inference.py' in the python folder to test the model on the board.
...
```shell
hrt_model_exec perf --model_file ./model/efficientnet_lite1_240x240_nv12.hbm \
                                     --core_id=0 \
                                     --frame_count=200 \
                                     --perf_time=0 \
                                     --thread_num=1
...
- Frame count is: 200
Perf result:
- Frame totally latency is: 97.857 ms
- Average latency is: 0.489 ms
- Frame rate is: 1948.957 FPS
```
thread_num = 3
```
Running condition:
...
- Frame count is: 200
Perf result:
- Frame totally latency is: 141.678 ms
- Average latency is: 0.708 ms
- Frame rate is: 4086.470 FPS
```
...
```shell
hb_compile --model tf_efficientnet_lite1.onnx --march nash-e
```
...
```shell
hb_compile --config yaml/efficientnet_lite1_config.yaml
```

After compilation, you will find the output in the model_output directory. The file needed for deployment is efficientnet_lite1_240x240_nv12.hbm.
...
Summary:
FPS (1 core): 3289.88
latency: 0.3 ms (304 us)
BPU conv original OPs per run: 1,200,370,464
```
...
* [s100_inference.py](python/s100_inference.py) supports HBM format for inference on the board. using the new HB_HBMRuntime API using the new HB_HBMRuntime API

x86_inference.py requires -m and -i to specify the model and image paths, for example:

```shell
python3 python/x86_inference.py -m model_output/efficientnet_lite1_240x240_nv12_quantized_model.bc -i data/zebra_cls.jpg
```

To perform accuracy validation with `x86_inference.py`, use the `--validate` flag to enable validation mode. For example:

```shell
python3 python/x86_inference.py -m model_output/efficientnet_lite1_240x240_nv12_quantized_model.bc --validate -d ../../../imagenet/val -l ../../../imagenet/val.txt
```
