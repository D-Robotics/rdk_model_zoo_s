...
|----------------------|--------------------|---------|------------|------------|------------|-------------------------------------|------------------------------------|-------------|
| EfficientNet_lite3   | 300x300            | 1000    | 8.2        | 79.8       | 79.2          | 0.668 ms                            | 1.249 ms                           | 2345.518 FPS |
...
```shell
wget https://archive.d-robotics.cc/downloads/rdk_model_zoo/rdk_s100/EfficientNet/efficientnet_lite3_300x300_nv12.hbm
```
...
After installing the necessary libraries, you can use the [get_efficientnet_lite3_onnx.py](python/get_efficientnet_lite3_onnx.py) script in the python folder to download the ONNX file.
...
* If you do not want to configure a terminal proxy, you can manually download the model from [timm/tf_efficientnet_lite3.in1k](https://huggingface.co/timm/tf_efficientnet_lite3.in1k) and use the [python/timm2onnx.py](python/timm2onnx_local.py) script to convert to ONNX.
...
Simplified model saved to tf_efficientnet_lite3.onnx
Total number of parameters in the model: 8158762
```

## 4. Deployment Test

After downloading the .hbm file, you can run 'test_EfficientNet_lite3.ipynb' or 's100_inference.py' in the python folder to test the model on the board.
...
```shell
hrt_model_exec perf --model_file ./model/efficientnet_lite3_300x300_nv12.hbm \
                                     --core_id=0 \
                                     --frame_count=200 \
                                     --perf_time=0 \
                                     --thread_num=1
...
- Frame count is: 200
Perf result:
- Frame totally latency is: 133.566 ms
- Average latency is: 0.668 ms
- Frame rate is: 1451.031 FPS
```
thread_num = 3
```
Running condition:
...
- Frame count is: 200
Perf result:
- Frame totally latency is: 249.863 ms
- Average latency is: 1.249 ms
...
```shell
hb_compile --model tf_efficientnet_lite3.onnx --march nash-e
```
...
```shell
hb_compile --config yaml/efficientnet_lite3_config.yaml
```

After compilation, you will find the output in the model_output directory. The file needed for deployment is efficientnet_lite3_300x300_nv12.hbm.
...
Summary:
FPS (1 core): 1778.44
latency: 0.56 ms (562.3 us)
BPU conv original OPs per run: 3,215,109,088
```
