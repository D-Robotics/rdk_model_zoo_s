...
def process_mask(protos: np.ndarray, 
    ...
    """Process masks with correct Letterbox/Padding removal.

    Args:
        protos: Prototype masks (32, 160, 160).
        masks_in: Mask coefficients (N, 32).
        bboxes: Bounding boxes (N, 4) in Model Input Scale (640x640).
        shape: Original image shape (orig_h, orig_w).
        input_h: Model input height (e.g. 640).
        input_w: Model input width (e.g. 640).
        resize_type: 0 (Stretch), 1 (Letterbox).
    """
    c, mh, mw = protos.shape  # (32, 160, 160)
    orig_h, orig_w = shape

    # 1. Matrix Multiplication -> (N, 160, 160)
    masks = (masks_in @ protos.reshape(c, -1)).reshape(-1, mh, mw)
    masks = sigmoid(masks)

    # 2. Resize masks to Model Input Size (e.g., 640x640)
    # This aligns the mask pixels with the input_tensor coordinates
    masks_input_scale = []
    for m in masks:
        # Use linear for smoother masks, nearest for speed
        m_resized = cv2.resize(m, (input_w, input_h), interpolation=cv2.INTER_LINEAR)
        masks_input_scale.append(m_resized)
    masks = np.array(masks_input_scale) # Shape (N, 640, 640)

    # 3. Crop masks to Bounding Boxes (still in Model Input Scale)
    # Doing this BEFORE un-padding ensures we don't process noise
    masks = crop_mask(masks, bboxes)

    # 4. Handle Resize/Un-padding (The Critical Fix)
    if resize_type == 0:  # Stretch
        # Simple resize to original
        final_masks = []
        for m in masks:
            m_orig = cv2.resize(m, (orig_w, orig_h), interpolation=cv2.INTER_LINEAR)
            final_masks.append(m_orig)
        masks = np.array(final_masks)

    elif resize_type == 1:  # Letterbox
        # Calculate the padding that was added during pre-processing
        scale = min(input_w / orig_w, input_h / orig_h)
        
        # New width/height of the valid image area inside the 640x640 tensor
        nw = int(orig_w * scale)
        nh = int(orig_h * scale)
        ...
        # Crop the valid area from the 640x640 mask (Remove black bars)
        # Shape becomes (N, nh, nw)
        masks_cropped = masks[:, pad_h : pad_h + nh, pad_w : pad_w + nw]
        
        # Resize the valid area to Original Image Size
        final_masks = []
...
class YOLO26Seg:
    ...
    def post_process(self,
        ...
        full_masks = process_mask(proto_tensor, mask_coefs, xyxy, 
                             (ori_img_h, ori_img_w), 
        ...
        # Step 6: Crop Masks (Adaptation for visualize.draw_masks)
        # Instead of returning (N, H, W), we return a list of crops
        cropped_masks = []
        for i, box in enumerate(xyxy):
            ...
            # Slice the full mask
            # Check validity to avoid empty slice errors
            if x2 > x1 and y2 > y1:
                m = full_masks[i][y1:y2, x1:x2]
            else:
                m = np.zeros((0, 0), dtype=bool) # Empty mask
