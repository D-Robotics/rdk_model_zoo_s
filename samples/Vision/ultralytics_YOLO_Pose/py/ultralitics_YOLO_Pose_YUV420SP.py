...
def main():
    ...
    parser.add_argument('--model-path', type=str, default='source/reference_hbm_models/yolo11x_pose_nashe_640x640_nv12.hbm', 
    ...
    if not os.path.exists(opt.model_path):
        ...
        os.system("wget -c https://archive.d-robotics.cc/downloads/rdk_model_zoo/rdk_s100/ultralytics_YOLO/yolo11n_pose_nashe_640x640_nv12.hbm")
        opt.model_path = 'yolo11n_pose_nashe_640x640_nv12.hbm'

    # 实例化
    model = YOLO11_Pose(opt)
    # 读图
    ...
    for score, x1, y1, x2, y2, kpt_xy, kpt_score in results:
        logger.info("(%d, %d, %d, %d) -> %s: %.2f"%(x1,y1,x2,y2, coco_names[0], score))
        draw_detection(img, (x1, y1, x2, y2), score, 0)
        for j in range(17):
...
class YOLO11_Pose():
    ...
    def postProcess(self, outputs):
        ...
        # nms
        indices = cv2.dnn.NMSBoxes(dbboxes, scores, self.SCORE_THRESHOLD, self.NMS_THRESHOLD)
        ...
        for indic in indices:
            ...
            kpt_xy = kpts_xy[indic, :, :] 
            kpt_xy[:,0] = (kpt_xy[:,0] - self.x_shift) / self.x_scale
            kpt_xy[:,1] = (kpt_xy[:,1] - self.y_shift) / self.y_scale
            kpt_score = kpts_score[indic, :, :]

            results.append((scores[:][indic], x1, y1, x2, y2, kpt_xy, kpt_score))
