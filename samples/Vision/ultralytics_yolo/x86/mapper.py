...
def run_nash(opt):
    ...
    try:
        ...
        logger.info("hb_compile is available and working.")
    except (subprocess.CalledProcessError, FileNotFoundError):
        logger.error("hb_compile is not available.")
        exit(1)
    ...
    # 获取模型文件名用于生成输出文件名
    model_name = os.path.splitext(os.path.basename(opt.onnx))[0]
    march_str = {'bayes-e': 'bayese', 'nash-p': 'nashp', 'nash-m': 'nashm', 'nash-e': "nashe"}[opt.march]
...
'''
    ...
    for img_name in img_names:
        ...
        # S100的前处理以ONNX的前处理一致，总的来说是和训练时的前处理保持一致
        input_tensor = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)     # BGR2RGB
        input_tensor = cv2.resize(input_tensor, (width, height)) # resize
        input_tensor = np.transpose(input_tensor, (2, 0, 1))    # HWC2CHW
        input_tensor = np.expand_dims(input_tensor, axis=0).astype(np.float32) / 255.0 # CHW -> NCHW
    ...
    try:
        ...
        # mapper conversion
        cmd = f"hb_compile --config config.yaml"
        logger.info(f"Executing model conversion...")
        logger.info(f"Command: {cmd}")
        result = os.system(cmd)
        if result != 0:
        ...
        mapper_log_source = os.path.join(opt.ws, "hb_compile.log")
        mapper_log_dest = os.path.join(opt.output_dir, "hb_compile.log")
        
        if os.path.exists(mapper_log_source):
            shutil.move(mapper_log_source, mapper_log_dest)
            logger.info(f"hb_compile log moved to: {mapper_log_dest}")
        else:
            logger.warning(f"hb_compile log not found at: {mapper_log_source}")
